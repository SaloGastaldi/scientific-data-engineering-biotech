...
def leer_camion(nombre_archivo_camion):
    camion = fileparse.parse_csv(nombre_archivo_camion, select = ['nombre', 'cajones', 'precio'], types = [str, int, float])
    return camion

def leer_precios(nombre_archivo_precios):
    lista_precios = fileparse.parse_csv(nombre_archivo_precios, types = [str, float], has_headers = False)
